...
using dyClawler.Common;
using dyClawler.Helper;
...
namespace dyClawler.Crawler
{
    public class Dy2018MovieCrawler
    {
        private static luofu_interestshareDbcontext MovieDataContent { get; } = new luofu_interestshareDbcontext();
        ...
        public static void CrawlMovieInfo()
        {
            Task.Factory.StartNew(() =>
            {
                try
                {
                    var indexPageCount = MovieDataContent.crawlerconfigconfiguration
                    ...
                    MovieDataContent.crawlerconfigconfiguration.Where(c => c.IsEnabled && c.ConfigconfigurationName == ConstsConf.Dy2018CrawlerList)
                    .ForEach(config =>
                    {
                        //取前五页
